...
test_loader = DataLoader(
    test_dataset[graph_mask], batch_size=1, shuffle=False, drop_last=False
)
...
refine_model_path = f"gnns/{gnns_dict[args.dataset]}.py"
refine = torch.load(refine_model_path)
refine.remap_device(device)

# ---------------------------------------------------
in_features = torch.flatten(test_dataset[0].x, 1, -1).size(1)
mlp_model = GLNN(device=device, in_features=in_features, out_features=1)
...
for epoch in range(10):
    ...
    for g in tqdm(iter(test_loader), total=len(test_loader)):
        g.to(device)
        z = refine.explain_graph(
            g, fine_tune=True, to_numpy=False, ratio=0.5, lr=args.lr, epoch=args.epoch
        )

        optimizer.zero_grad()
        y_hat = mlp_model(g.x)
        loss = loss_lambda * loss_label(y_hat, g.y) + (1 - loss_lambda) * loss_teacher(
            y_hat, z
        )
        loss.backward()
        optimizer.step()
